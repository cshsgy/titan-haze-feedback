...
structure but a stratosphere $\sim\!80\,\mathrm{K}$ cooler than observed. That
deficit is the signature of the gray shortwave gas: with full correlated-$k$
methane the upper atmosphere would absorb more sunlight (Sec.~\ref{sec:rt-fortran}).
The real-CIA longwave, energy closure, and the
...
$\sim\!110\,\mathrm{K}$). The heating panel shows why: the reference model's
spectral methane absorbs substantially more sunlight in the stratosphere than our
gray placeholder, so its shortwave heating (and hence stratospheric temperature)
is larger. The close agreement on the longwave-controlled lower atmosphere, and
the structural agreement throughout, indicate that the real-CIA longwave is the
dominant control on the thermal structure and that the stratospheric temperature
gap is attributable to the shortwave gas treatment---i.e.\ replacing the gray
shortwave with correlated-$k$ methane is the priority upgrade.
