...
the heating. Because the opacities match where the heating rates differ, the
mid-atmosphere discrepancy is \emph{not} an opacity error: it is attributable to
the radiative-transfer solver itself---our eight-stream discrete-ordinates
treatment versus the reference's delta-Eddington two-stream (shortwave) and
hemispheric-mean (longwave), which handle the strongly forward-scattering haze
($\omega_0\approx0.9$ in the visible) differently. The breakdown also exposes a
genuine opacity difference confined to the very top ($P<10\,\mathrm{Pa}$), where
our haze column is thinner than the reference's---an upper-atmosphere effect
distinct from the mid-altitude solver difference.
