...
Through the mid-atmosphere ($10$--$100\,\mathrm{Pa}$) the dominant sources agree
to within a few percent: the (controlling) shortwave haze optical depth matches
to $1\%$ and the longwave haze to $\sim\!10\%$. The CH$_4$ shortwave gas is the
one mid-atmosphere outlier, ours $\sim\!1.65\times$ the reference, but that band
is optically thin ($\tau\lesssim0.01$ in the column) and contributes negligibly
to the heating. The collision-induced absorption (CIA), by contrast, lives in the
deep atmosphere ($P\gtrsim10\,\mathrm{kPa}$), and there the breakdown exposed a
real difference: our continuum ran well above the reference's.
...
$0.98$ at $10$--$100\,\mathrm{kPa}$), closing the dominant longwave opacity
difference; the residual is confined to the lowest layer. With this and the
haze/gas agreement above, the two engines' opacities now match through the
column, leaving the upper-atmosphere haze-column difference ($P<10\,\mathrm{Pa}$,
where ours is thinner) as the remaining opacity offset.
