...
properties combine the haze---taken directly from the Step~2 microphysics via a
geometric cross-section $Q_{\rm ext}\pi r_a^2$, with $\omega_0,g$ from the
Huygens/DISR literature (scattering in the shortwave, a near-pure absorber in
the IR)---and the gas.
...
right), which is the origin of Titan's pressure-induced greenhouse. The
\emph{shortwave} gas opacity remains a simple gray CH$_4$-like placeholder
(distributed by mass column), the one remaining approximation, to be replaced
with correlated-$k$ methane coefficients.

On the Titan reference column the solver reproduces the budget:
$\sim\!\SI{2.8}{W.m^{-2}}$ of shortwave is absorbed, the outgoing longwave is
$\sim\!\SI{4.4}{W.m^{-2}}$, and the stratospheric cooling rates are of Tomasko
order (a few $\mathrm{K}$ per Titan day). Relaxing the \emph{layer-mean}
temperatures along the net heating rate (under-relaxed, surface held fixed; the
layer-mean state is in 1:1 correspondence with the per-layer heating, avoiding
the level$\leftrightarrow$layer aliasing that otherwise seeds a grid-scale
zig-zag) cuts the bulk net-heating residual by over an order of magnitude
($\sim\!12\,\mathrm{K}$ per Titan day) and yields a recognizably
Titan-like profile (Fig.~\ref{fig:ebal}): a cold tropopause near
$74\,\mathrm{K}$ and a warm stratopause near $190\,\mathrm{K}$ (cf.\ the observed
$\sim\!186\,\mathrm{K}$). The relaxation reaches a stable quasi-steady state
rather than tight radiative equilibrium---the gray shortwave deposition is
concentrated too high, which spectral methane opacity should relieve---but the
real CIA longwave and the haze$\rightarrow$optics$\rightarrow$DISORT coupling are
in place, which is what Step~3 requires.
...
plus N$_2$--N$_2$/N$_2$--CH$_4$/CH$_4$--CH$_4$/N$_2$--H$_2$ CIA) with
prescribed haze, run to radiative--convective equilibrium on a $100$-layer
column. Figure~\ref{fig:fortran} overlays its temperature and heating/cooling
profiles on ours. The two temperature structures agree closely---stratopause
$\sim\!185$--$190\,\mathrm{K}$, cold tropopause $\sim\!70$--$75\,\mathrm{K}$,
surface $\sim\!90\,\mathrm{K}$---despite our simplified gray shortwave gas; the
residual differences are largest in the mid-stratosphere ($10^2$--$10^3\,\mathrm{Pa}$),
where the reference model's spectral methane heating is stronger aloft (visible
in the heating panel) than our mass-distributed gray placeholder. This supports
the conclusion that the real-CIA longwave is the dominant control on the thermal
structure and that the remaining gap is attributable to the shortwave gas
treatment.
