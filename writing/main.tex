...
The solver reaches a genuine steady state with $\sim\!\SI{4.0}{W.m^{-2}}$ of
shortwave absorbed (close to the $\sim\!\SI{4.5}{W.m^{-2}}$ of
\citealp{Tomasko2008heat}), energy closed to $\sim\!0.1\,\mathrm{W\,m^{-2}}$.
Driven by the Step~2 microphysics haze it has the right vertical structure---a
stratopause ($\sim\!300\,\mathrm{Pa}$), a cold tropopause near $74\,\mathrm{K}$,
a $\sim\!92\,\mathrm{K}$ surface---but a stratosphere $\sim\!80\,\mathrm{K}$ too
cold (Fig.~\ref{fig:ebal}, red).

To localize this deficit we ran the \emph{same} radiative engine on the
...
so the radiation physics (correlated-$k$ CH$_4$ shortwave, spectral haze, CIA,
correlated-$k$ gas-line longwave, convective adjustment, energy closure) is
\emph{correct}, and the entire $\sim\!80\,\mathrm{K}$ cold bias is the
microphysics haze: our Step~2 haze places too much optical depth too low and too
much in total (the mobility-radius optical cross-section overestimates large-
aggregate extinction, giving a column optical depth several times observed). This
is a Step~2 microphysics$\to$optics calibration issue (mean-field aggregate
optics), not a radiation one. The validated radiation engine, energy closure, and
haze$\rightarrow$optics$\rightarrow$DISORT coupling are in place, which is what
Step~3 requires.
...
coordinate. Driven by the Step~2 microphysics haze (red), our model has the right
structure but a $\sim\!115\,\mathrm{K}$ stratopause versus the reference's
$\sim\!195\,\mathrm{K}$. Driven instead by the \emph{prescribed} haze (green)---
the observational optical depths the reference uses, in the same radiative
engine---our model reaches $\sim\!200\,\mathrm{K}$ and tracks the reference
(black) closely through the stratosphere and mesosphere. This isolates the
discrepancy cleanly: the radiation is right (the engine reproduces the reference
given the same haze), and the cold bias of the coupled run is entirely the
microphysics haze---its optical depth is too large and deposited too low (the
mobility-radius optical cross-section overestimates large-aggregate extinction).
The remaining upgrade is therefore in Step~2 (mean-field aggregate optics for the
haze cross-section), not in the radiation.
