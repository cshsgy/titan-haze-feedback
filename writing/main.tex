...
The solver reaches a genuine steady state: $\sim\!\SI{2.8}{W.m^{-2}}$ of
shortwave is absorbed against $\sim\!\SI{2.9}{W.m^{-2}}$ of outgoing longwave
(energy closed to $\sim\!0.1\,\mathrm{W\,m^{-2}}$), with a converged net-heating
residual below $1\,\mathrm{K}$ per Titan day. The equilibrium profile
(Fig.~\ref{fig:ebal}) is recognizably Titan-like---a stratopause near
$110\,\mathrm{K}$ ($\sim\!300\,\mathrm{Pa}$), a cold tropopause near
$74\,\mathrm{K}$, and a $\sim\!92\,\mathrm{K}$ surface---with the right vertical
structure but a stratosphere still $\sim\!80\,\mathrm{K}$ cooler than observed.
Adding correlated-$k$ methane (above) warms the stratopause only modestly
($\sim\!108\to115\,\mathrm{K}$): although CH$_4$ contributes real spectral
heating, the total shortwave absorbed is unchanged because on Titan the
\emph{haze} is the dominant shortwave absorber. The residual cold bias therefore
traces to the haze shortwave treatment---our microphysical haze deposits its
absorption lower in the column (its mass peaks near the surface) than the
reference model's observationally-prescribed stratospheric haze
(Sec.~\ref{sec:rt-fortran})---so observationally-constrained, spectrally-resolved
haze optics is the next refinement. The correlated-$k$ shortwave methane, real-CIA
longwave, energy closure, and haze$\rightarrow$optics$\rightarrow$DISORT coupling
are in place, which is what Step~3 requires.
...
$\sim\!115\,\mathrm{K}$). Both models now use correlated-$k$ methane in the
shortwave (the same $k$-table), so the gap is \emph{not} the gas: the heating
panel shows the reference model deposits more shortwave in the stratosphere
because its observationally-prescribed haze absorbs more sunlight there than our
microphysical haze, whose absorption (with $\omega_0\!=\!0.92$ and a mass profile
peaking near the surface) is weaker and lower. The close agreement on the
longwave-controlled lower atmosphere indicates the real-CIA longwave is the
dominant control on that part of the structure; the stratospheric temperature
gap is now attributable to the \emph{haze shortwave optics}, making
observationally-constrained, spectrally-resolved haze single-scattering
properties the priority upgrade.
