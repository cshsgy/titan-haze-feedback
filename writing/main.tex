...
($62$ vs.\ $65\,\mathrm{km}$) and a sub-micron characteristic radius consistent
...
binding) in a two-band configuration: a collimated solar beam in the shortwave
and a Planck thermal source in the longwave, with eight streams. Per layer the
optical properties $(\tau,\omega_0,g)$ combine the haze---taken directly from the
Step~2 microphysics via a geometric cross-section $Q_{\rm ext}\pi r_a^2$, with
$\omega_0,g$ from the Huygens/DISR literature (scattering in the shortwave, a
near-pure absorber in the IR)---and the gas. In this first cut the gas opacity is
a deliberately simple \emph{gray} placeholder for the correlated-$k$ CH$_4$+CIA
opacity described above: a shortwave (CH$_4$-like) component distributed by mass
column, and a longwave component blending a CIA-like ($\propto n^2$) term with a
line-like ($\propto$ mass) term so the low-density upper stratosphere retains
emissivity. This is the principal approximation of the present model and the
first target for replacement with real $k$-tables.

On the Titan reference column the solver reproduces the qualitative budget:
$\sim\!\SI{2.8}{W.m^{-2}}$ of shortwave is absorbed, the outgoing longwave is
$\sim\!\SI{5.6}{W.m^{-2}}$, and the bulk ($\lesssim150\,\mathrm{km}$)
heating/cooling rates are of order Tomasko's (a few $\mathrm{K}$ per Titan day).
Relaxing level temperatures along the net heating rate (per-level,
under-relaxed, with the surface held fixed) cuts the bulk net-heating residual
by roughly an order of magnitude and yields a recognizably Titan-like profile
(Fig.~\ref{fig:ebal}): a cold tropopause near $70\,\mathrm{K}$ and a warm
stratosphere ($\sim\!185$--$190\,\mathrm{K}$, somewhat warm versus the observed
$\sim\!170$--$186\,\mathrm{K}$). The relaxation reaches a stable quasi-steady
state rather than tight radiative equilibrium: with only two gray bands a
shortwave-absorbing layer cannot always cool exactly locally, an artifact that
spectral resolution (Step~1 refinement) is expected to remove. The machinery and
the haze$\rightarrow$optics$\rightarrow$DISORT coupling are in place, which is
what Step~3 requires.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/energy_balance.png}
  \caption{Two-band DISORT energy balance on the Titan column with the Step~2
    haze. Left: shortwave heating, longwave cooling, and net rate at the
    relaxed state. Centre: net downward shortwave and longwave fluxes. Right:
    initial-guess vs.\ radiative-equilibrium temperature (surface fixed).}
  \label{fig:ebal}
\end{figure}
...
$n\,r_a^2$ over $80$--$300\,\mathrm{km}$ yields $H=\SI{62}{km}$, against the
